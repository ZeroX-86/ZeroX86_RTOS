...
static timer_elect_t OS_SysTimer;
...
OS_ReturnT OS_SysInit()
{
	...
	systick_init(OS_SysTimer,1000,OS_SysTick);
}
						 ...
						 OS_Uint8T Priority)
{
	...
	if (PtrTemp != OS_NULL)
	{
		PtrTemp->TaskHandler = PtrTemp;
		PtrTemp->TaskId = Id;
		PtrTemp->TaskName = Name;
...
static OS_VoidT OS_TaskExecute(OS_VoidT* PVArg)
{
	...
	{
		(*TempPtr->TaskHandler)(TempPtr->TaskArg);
